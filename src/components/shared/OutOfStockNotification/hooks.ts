...
export function useForm(handleToggle: () => void, currentVariantId: string) {
	...
	const handleUntouchedFields = useCallback(() => {
		...
		if (isAnyInputInvalid) {
			const textInputIds = ["firstName", "email"];
	...
	const handleSubmit = useCallback(
		...
		[formState.mandatoryInputs, handleUntouchedFields, currentVariantId, handleToggle],
	);
