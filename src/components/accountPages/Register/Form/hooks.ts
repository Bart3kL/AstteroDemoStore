...
import { register } from "@/lib/shopify/functions/account/actions";
...
export function useForm() {
	...
	const handleUntouchedFields = useCallback(() => {
		...
		if (isAnyInputInvalid) {
			const textInputIds = ["firstName", "lastName", "password", "email"];
	...
	const handleSubmit = useCallback(
		async (event: any) => {
			...
			try {
				...
				try {
					const data = {
						email: mandatoryInputs.email.value,
						password: mandatoryInputs.password.value,
						firstName: mandatoryInputs.firstName.value,
						lastName: mandatoryInputs.lastName.value,
					};
